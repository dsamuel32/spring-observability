...
public class SearchArtistUseCase implements UseCase<Search, Pageable<ArtistOutput>> {

    private final Logger LOGGER = Logger.getLogger(SearchArtistUseCase.class.getName());

    private final ArtistGateway gateway;
    @Override
    public Pageable<ArtistOutput> execute(final Search search) {
        LOGGER.log(Level.INFO, "[SEARCH ARTIST][START] - {}", search);
